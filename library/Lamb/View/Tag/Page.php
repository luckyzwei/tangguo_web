...
class Lamb_View_Tag_Page extends Lamb_View_Tag_Abstract
{
	...
	public function parse($content, $property)
	{
		...
		if (preg_match($strPageItemPatt, $content, $aPageItemMatch, PREG_OFFSET_CAPTURE)) {
			...
			$strPageStart	.=	self::parseVar(self::codeAddslashes(substr($content, 0, $nPageItemStart)));
		...
		if ($isAttrExists){
			$strPageEnd	.=	self::parseVar(self::codeAddslashes(substr($content, $nPageItemStart)));
		}
		else{
			$strPageStart.=	self::parseVar(self::codeAddslashes(substr($content,$nPageItemStart)));;
		}
