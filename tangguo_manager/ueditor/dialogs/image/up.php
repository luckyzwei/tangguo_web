...
<?php
$g_aUploadConfig = require_once '../../../config.php';
    if(empty($_GET['submit'])){
        $path=$g_aUploadConfig['pic_path']; 
		...
		if(!in_array($current_type, $g_aUploadConfig['allow_upload_pic_suffix'])){
		...
		$file_size = 1024 * $g_aUploadConfig['max_upload_file_size'];
		if( $_FILES["filename"]["size"] > $file_size ){
			echo "<script type='text/javascript'>alert('图片大小超出".$g_aUploadConfig['file_size']."KB限制，请重新选择！')</script>";
			echo "<script type='text/javascript'>location.href='upload.php'</script>";
			exit;
		}
		...
		if($result){
			...
			echo "<script type='text/javascript'>location.href='upload.php?url=".urlencode($_POST['path'])."'</script>";
		}
